...
def f(i, N, lw, rw):    # i: 저울에 올린 추 개수, N: 추 개수, lw: 왼쪽 무게, rw: 오른쪽 무게
    if lw < rw:         # 오른쪽 무게가 더 큰 조건이라면
        return 0         # 조건에 위배되는 경우
    elif i == N:        # 모든 추를 올린 경우
        return 1
    else:
        result = 0                     # i개의 추를 놓았을 때
        for j in range(i, N):
            p[i], p[j] = p[j], p[i]    # i 순서에 올린 추 번호 p[i]정하기
            result += f(i+1, N, lw + arr[p[i]], rw)   # p[i] 추를 왼쪽에 올린 경우
            result += f(i+1, N, lw, rw + arr[p[i]])   # p[i] 추를 오른쪽에 올린 경우
